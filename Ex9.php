...
function mostrarValorAlmacen() {
    ...
    // Recorre cada categoría en el inventario
    foreach ($inventario as $categoria) {
        // Recorre cada cómic dentro de la categoría
        foreach ($categoria as $comic) {
            // Calcula el valor total del cómic (precio * stock)
            $totalValor += $comic['precio'] * $comic['stock'];
        }
    }

    // Muestra el valor total del almacén
    echo "El valor total del almacén es: " . $totalValor;
}
